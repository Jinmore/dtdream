...
class CategoryModel extends Model{
	 ...
	 * @author 麦当苗儿 <zuojiazi@vip.qq.com>
	 */
	public function getChildrenId111($cate){
	...
	// 改过的方法：
	public function getChildrenId($cate,$model=0){
			$field = 'id,name,pid,title,link_id,is_page,display';
			$category = D('Category')->getTree($cate, $field, $model);
			$ids[]    = $cate;
			/*if($model){
					foreach ((array)$category as $key2 => $value2) {
							if($value2['id']) $ids[] = $value2['id'];
							foreach ((array)$value2['_'] as $key3 => $value3) {
									$ids[] = $value3['id'];
							}
					}
			}else{
					foreach ((array)$category['_'] as $key => $value) {
