 ...
 */
// 生成訂單接口
function create_order($akId,$secret,$url,$data)
{
  $param = array(
  ...
  ksort($param);
  $param_string = 'POST' . '&%2F&' . urlencode(http_build_query($param));
  $signature = base64_encode(hash_hmac('sha256', $param_string, $secret . '&', true));
  $ch = curl_init($url);
