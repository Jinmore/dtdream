...
class ArticleController extends AdminController {
    ...
    public function index($cate_id = null){
        ...
        foreach ($grids as &$value) {
            ...
            if(isset($val[2])){
                // 链接信息
								//  var_dump($val[2]);
								 if($val[2]=="article/index?cate_id=[category_id]&pid=[id]")
								 continue;
                $value['href']  =   $val[2];
    ...
    public function edit(){
        ...
        if($data['pid']){
            // 获取上级文档
            $article        =   D('Document')->field('id,title,type')->find($data['pid']);
            $this->assign('article',$article);
        }
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = M('Document')->where($map)->field('id,title,level')->order('level DESC,id DESC')->select();
