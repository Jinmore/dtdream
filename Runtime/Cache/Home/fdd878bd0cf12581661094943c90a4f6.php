...
<div class="news-main">
    <div class="inner">
        ...
        <ul class="gao">
         <?php $category=D('Category')->getChildrenId($_GET['catid']);$__LIST__ = D('Document')->page(1,3)->lists($category, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?><li class="clearBoth">
                <a href="<?php echo U('Article/detail?id='.$article['id'].'&catid='.$_GET['catid']);?>" class="fl"><?php echo ($article['title']); ?></a>
