...
<div class="news-main">
    <div class="inner">
        <div class="base-line clearBoth">
            ...
            <ul class="fr clearBoth">
              <?php for($i=87;$i<=89;$i++):?>
                <li <?php if($_GET['catid']==$i) echo "class='bg'";?> ><a href="<?php echo U('Article/index',array('category'=>get_category($i,'name'),'catid'=>$i));?>"><?php echo get_category($i,'title');?></a></li>
              <?php endfor;?>
            </ul>
        </div>
        <ul class="newsUl">
          <?php $category=D('Category')->getChildrenId($_GET['catid']);$__LIST__ = D('Document')->page(1,3)->lists($category, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li class="clearBoth">
                <a href="<?php echo U('Article/detail?id='.$list['id'].'&catid='.$_GET[catid]);?>" class="left fl"><img src="<?php echo get_cover($list['cover_id'],$field = path);?>"></a>
                <div class="right fl">
                    <a href="<?php echo U('Article/detail?id='.$list['id'].'&catid='.$_GET['catid']);?>" class="title"><?php echo ($list['title']); ?></a>
                    <p><?php echo (date('Y-m-d',$list["create_time"])); ?></p>
                    <div class="text">
                      <?php echo ($list['description']); ?><a href="<?php echo U('Article/detail?id='.$list['id'].'&catid='.$_GET['catid']);?>">[详细]</a>
                    </div>
                </div>
            </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
        <div class="page">
    			<?php $__CATE__ = D('Category')->getChildrenId($_GET['catid']);$__PAGE__ = new \Think\Page(get_list_count($__CATE__), 3);echo $__PAGE__->show(); ?>
    		</div>
    </div>
</div>
<div class="footer">
