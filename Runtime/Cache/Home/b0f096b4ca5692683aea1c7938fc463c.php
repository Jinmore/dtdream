...
<div class="inner">
    ...
    <div class="header">
        ...
        <form class="registerform11 fl clearBoth"  name="search_form" method="get" action="<?php echo U('Article/lists',array('category'=>get_category(82,'name')),true,true);?>">
            <input type="text" placeholder="请输入关键字" name="keyword" id="keyword" class="fl" datatype="*" nullmsg="请输入搜索内容" value="<?php echo I('keyword');?>">
...
<div class="news-main">
    <div class="inner">
        ...
        <ul class="gao">
         <?php $category=D('Category')->getChildrenId(73);$__LIST__ = D('Document')->page(1,10)->lists($category, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?><li class="clearBoth">
                <a href="<?php echo U('Article/detail?id='.$article['id'].'&catid=73');?>" class="fl"><?php echo ($article['title']); ?></a>
                <div class="time fr"><?php echo (date('Y-m-d',$article["create_time"])); ?></div>
            </li><?php endforeach; endif; else: echo "" ;endif; ?>
          <!-- <?php $category=D('Category')->getChildrenId($category['id']);$__LIST__ = D('Document')->page(1,$category['list_row'])->lists($category, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="lie-lvse-lianmeng">
      				<h5><a href="<?php echo U('Article/detail?id='.$list['id']);?>"><?php echo ($list["title"]); ?></a><span><?php echo (date('Y-m-d',$list["create_time"])); ?></span></h5>
      				<div class="jian"><?php echo ($list["description"]); ?></div>
      			</div><?php endforeach; endif; else: echo "" ;endif; ?> -->
        </ul>
        <div class="page" >
					 <?php $__CATE__ = D('Category')->getChildrenId(73);$__PAGE__ = new \Think\Page(get_list_count($__CATE__), 10);echo $__PAGE__->show(); ?>
				</div>
    </div>
</div>
<div class="footer">
