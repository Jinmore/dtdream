...
<div class="news-main">
    <div class="inner">
        <div class="base-line clearBoth">
            ...
            <p class="fl"><?php echo get_category($pid,'title');?></p>
            <ul class="fr clearBoth">
              <?php if($pid==53):?>
              <?php for($i=67;$i<=68;$i++):?>
        ...
        <ul class="yun ke clearBoth">
          <?php $category=D('Category')->getChildrenId($_GET['catid']);$__LIST__ = D('Document')->page(1,12)->lists($category, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ketang): $mod = ($i % 2 );++$i;?><li>
               <a href="<?php echo U('Article/detail?id='.$ketang['id'].'&catid='.$_GET['catid']);?>"><img src="<?php echo get_cover($ketang['cover_id'],$field = path);?>"></a>
               <div class="text">
                   <a href="<?php echo U('Article/detail?id='.$ketang['id'].'&catid='.$_GET['catid']);?>"><?php echo (msubstr($ketang['title'],0,8)); ?></a>
                   <?php echo (msubstr($ketang[description],0,45)); ?>
               </div>
               <div class="bot clearBoth">
                   <div class="a2 fl"><?php echo ($ketang['price']); ?>元起</div>
                   <a href="<?php echo U('Article/detail?id='.$ketang['id'].'&catid='.$_GET['catid']);?>" class="a1 fr">立即选购</a>
               </div>
           </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
        <div class="page">
    			<?php $__CATE__ = D('Category')->getChildrenId($_GET['catid']);$__PAGE__ = new \Think\Page(get_list_count($__CATE__), 12);echo $__PAGE__->show(); ?>
    		</div>
    </div>
</div>
<div class="footer">
