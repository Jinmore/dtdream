...
<head>
    <meta charset="UTF-8">
    <title><?php echo ($meta_title); ?>|数梦科技管理平台</title>
...
<body>
    ...
    <div class="header">
        <!-- Logo -->
        <span class="logo"><img src="/Public/Admin/images/bg_icon11.png"></span>
        <!-- /Logo -->
        ...
        <div class="user-bar"><a href="<?php echo DOMAIN;?>" target="_blank" style="color:#fff">访问首页</a></div>
    </div>
    <!-- /头部 -->
	...
	<?php if($info['is_page'] == 1): ?><style>
			.news_lb,.url_div{
				display: none;
			}
		</style>
		<?php elseif($info['is_page'] == 2): ?>
		<style>
			.news_lb,.page_div{
				display: none;
			}
			...
			<form action="<?php echo U();?>" method="post" class="form-horizontal">
				...
				<div id="tab1" class="tab-pane in tab1">
					...
					<div class="form-item">
						<label class="item-label">
							栏目名称<span class="check-tips">（名称不能为空）</span>
						</label>
						<div class="controls">
					...
					<!-- <div class="form-item">
						...
						<div class="controls">
							<input type="text" name="title_en" class="text input-large" value="<?php echo ((isset($info["title_en"]) && ($info["title_en"] !== ""))?($info["title_en"]):''); ?>">
						</div>
					</div> -->
					<div class="form-item">
						<label class="item-label">
							分类标识<span class="check-tips">（英文字母，唯一）</span>
						</label>
						<div class="controls">
					...
					<div class="form-item">
						<label class="item-label">
							分类类型<span class="check-tips">（单网页，列表页，跳转url）</span>
						</label>
						<div class="controls">
							<label class="inline radio"><input type="radio" name="is_page" value="0" checked>列表</label>
							<label class="inline radio"><input type="radio" name="is_page" value="1">单页</label>
							<label class="inline radio"><input type="radio" name="is_page" value="2">外链</label>
						</div>
					</div>
					<div class="form-item url_div">
						<label class="item-label">
							跳转链接<span class="check-tips">（url网站，例如：http://www.baidu.com）</span>
						</label>
						<div class="controls">
							<input type="text" name="urll" class="text input-large" value="<?php echo ($info["urll"]); ?>">
						</div>
					</div>
					<div class="form-item news_lb">
						<label class="item-label">
					...
					<div class="form-item page_div">
						<label class="item-label">分类内容<span class="check-tips">（单页面显示的内容）</span></label>
						<div class="controls">
				...
				<div id="tab2" class="tab-pane tab2">
					...
					<div class="form-item page_div">
						...
						<div class="controls">
							<input type="text" name="template_page" class="text input-large" value="<?php echo ((isset($info["template_page"]) && ($info["template_page"] !== ""))?($info["template_page"]):''); ?>">
						</div>
					</div>
					<div class="form-item news_lb">
	...
	<script type="text/javascript">
		...
		$('input[name=is_page]').click(function(){
			var value = $(this).val();
			if(value == 1) {
				$('.news_lb').css('display', 'none');
				$('.url_div').css('display', 'none');
				$('.page_div').css('display', 'block');
				Think.setValue("allow_publish", 0);
			}else if(value == 2){
				$('.news_lb').css('display', 'none');
				...
				$('.page_div').css('display','none');
				$('.url_div').css('display', 'none');
				$('.news_lb').css('display', 'block');
			}
		});
		$('input[name=allow_publish]').click(function(){
			var value = $(this).val();
			if(value > 0) {
				$('.page_div').css('display','none');
				$('.news_lb').css('display', 'block');
				Think.setValue("is_page", 0);
			}
		});
		//导航高亮
